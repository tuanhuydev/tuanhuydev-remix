...
export const Header = () => {
  ...
  const Logo = () => (
    <div className="text-primary dark:text-slate-50 font-bold flex items-center">
      ...
      >
        ...
        {!showAtBottom && (
          <h1 className="inline text-base lg:text-2xl dark:text-slate-50">
            tuanhuydev
          </h1>
        )}
      </a>
  ...
  const Actions = () => (
    <div className="flex items-center gap-3">
      <button onClick={toggleTheme} className="p-2 rounded-full shadow-lg">
        {darkMode ? (
          <motion.svg
            initial={{ rotate: 0 }}
            animate={{ rotate: 360 }}
  ...
  return (
    <motion.header
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`rounded-full flex justify-between fixed z-10 px-3 py-2 left-1/2 -translate-x-1/2 bg-slate-50 dark:bg-slate-900 shadow-md 
        ${showAtBottom ? 'bottom-15 w-4/5 lg:w-1/5 ease-in' : 'top-4 w-4/5 lg:w-[992px] ease-in'}`}
    >
      <Logo />
      <Actions />
    </motion.header>
